...
(defn hostname
  "Returns a spec for an Internet hostname that conforms to RFC1123. Options may
   ...
   :tlds - the set of allowed top-level domains
   :min-depth - the minimum number of parts
   :max-depth - the maximum number of parts"
  [& options]
  (let [{:keys [tlds min-depth max-depth]} options
              ...
              (and (>= 253 (count s))
                   (let [parts (string/split s #"\.")]
                     (and (or (not max-depth)
                          ...
                          (or (not (seq tlds))
                              (contains? tlds (last parts)))))))
            (gen []
              (let [min-needed (or min-depth 2)
                    max-needed (or max-depth 4)
                    tld-part (when (seq tlds)
                               (gen/elements tlds))
                    tld-count (if tld-part 1 0)
                    parts-gen (gen/vector (s/gen hostpart-spec)
                                          (- min-needed tld-count)
                                          (- max-needed tld-count))]
                (if tld-part
                  (gen/let [tld tld-part
                            parts parts-gen]
                    (gen/return (string/join "." (conj parts tld))))
...
(def tlds
  (into #{} (map string/lower-case tld-list)))

(s/def ::fully-qualified-hostname
  (hostname :tlds tlds :min-depth 2))

(s/def ::fully-qualified-common-hostname
  (hostname :tlds #{"com" "edu" "net" "org"} :min-depth 2 :max-depth 4))
